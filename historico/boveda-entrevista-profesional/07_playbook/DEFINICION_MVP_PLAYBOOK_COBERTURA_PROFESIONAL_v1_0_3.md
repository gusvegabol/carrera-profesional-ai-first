# Definición MVP — Playbook de cobertura profesional (v1.0.3) (chatGPT)
...
Esta versión `v1.0.3` integra dos líneas previas de trabajo (La `v1` y la `v1.0.1`)
 
- la definición de cobertura profesional como mapa inicial, no exhaustivo pero consciente de sus límites;
    
- la arquitectura de cobertura como orquestador multi-sesión capaz de invocar el playbook de profundidad cuando detecta una zona fértil.
...
### 5.1. Zonas exploradas o cubiertas
...
Una zona cubierta permite decir:
...
- se aplicó una validación rápida, pero no una inmersión completa;
...
```text
Zona profesional
Estado: cubierta / candidata / parcial / pendiente
Evidencia asociada
...
## 9. Activación de profundidad
 
La cobertura puede activar el playbook de profundidad cuando una zona candidata muestra suficiente fertilidad.
 
La activación debe ser explícita.
 
Antes de entrar en profundidad, la IA debe poder decir algo equivalente a:
 
> Esta zona parece suficientemente rica como para trabajarla a fondo. Podemos entrar aquí y extraer una competencia con evidencia, o dejarla marcada como candidata y explorar otra zona.
 
La decisión debe respetar el consentimiento, la disponibilidad y el interés de la persona.
 
Una vez activada la profundidad:
 
- gobierna [[PLAYBOOK_ENTREVISTA_PROFESIONAL_v1_3_2_IA]];
    
- cobertura aporta contexto heredado;
    
- el presupuesto de tiempo declarado se conserva;
    
- no se repite la calibración de apertura salvo necesidad clara;
    
- al terminar, cobertura registra el resultado en el mapa.
...
### 12.1. Competencia profunda extraída, si existe
...
### 12.7. Siguiente exploración recomendada
...
## 13. Criterio mínimo de éxito MVP
...
## 14. Qué queda fuera de este MVP
...
## 15. Fricciones y antipatrones relacionados
...
## 17. Formulación operativa final
 
La cobertura profesional MVP es la capa de entrevista que permite construir, a través de una o varias sesiones, un mapa inicial y no exhaustivo de las zonas profesionales cubiertas, candidatas, parciales y pendientes de una persona.
...
```text
mapa → escaneo → zona candidata → validación ligera → profundidad si procede → actualización del mapa → siguiente zona
```
