...
## 10.1 Autorización editorial cerrada

La trazabilidad válida exige dos condiciones simultáneas:

```text
autorización editorial concreta del guion (A-NNN)
+
respaldo factual compatible con ese A-NNN
```

La existencia de un hecho en `datos-core-busqueda.md`, `candidatura.md` o `analisis-oferta.md` no lo convierte por sí sola en contenido narrable. El artefacto debe registrar un conjunto cerrado de afirmaciones autorizadas, cada una con un `claim_id`, uno o más `A-NNN`, los identificadores factuales compatibles y la frase visible autorizada.

El verificador debe comprobar que:

1. todo `claim_id` tiene al menos un `A-NNN` existente;
2. cada evidencia factual declarada está seleccionada por ese `A-NNN`;
3. toda afirmación profesional visible pertenece al conjunto cerrado;
4. una afirmación factual sin autorización editorial devuelve `requiere_correccion`.

Principio bloqueante:

> **factualmente verdadero no equivale a editorialmente autorizado.**
...
## T19 — Evidencia factual no seleccionada

Una evidencia existente en `datos-core-busqueda.md` pero ausente de todos los `A-NNN` autorizados no puede aparecer en la carta.

## T20 — Trazabilidad factual sin autorización editorial

Una frase con respaldo factual pero sin `A-NNN` debe devolver `requiere_correccion`.

## T21 — Bloque con referencia A-NNN insuficiente

Un bloque que cita un `A-NNN` pero añade una segunda afirmación profesional fuera de su conjunto autorizado debe detectarse.

## T22 — Auditoría factual insuficiente

La comprobación de que una afirmación existe en `datos-core-busqueda.md` no basta. La auditoría debe verificar simultáneamente `A-NNN` y fuente factual compatible.
...
Si la evidencia existía pero no estaba seleccionada por el guion, la corrección debe endurecer el conjunto cerrado y regenerar la instancia completa; no se debe editar el guion retroactivamente para justificarla.
